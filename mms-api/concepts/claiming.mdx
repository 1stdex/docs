...
- Trigger fee distribution to their provider and referrer
...
- Provider and referrer are stored with the order
...
- Taker's provider/referrer receive fees **immediately**
...
4. **Distribute** maker fees to your provider and referrer
...
- Maker fee: 0.1% (10 bp), referral rate: 40%
- Taker fee: 0.2% (20 bp), referral rate: 50%
...
- Bob's referrer: 2 USDC (50% of taker fee) ✅
- Bob's provider: 2 USDC (50% of taker fee) ✅
...
- To Alice's referrer: 2 × 40% = 0.8 USDC
- To Alice's provider: 2 × 60% = 1.2 USDC

Final state:
- Alice: 1000 TKN + 1998 USDC unlocked (net cost: 2 USDC fee)
- Alice's referrer: 0.8 USDC
- Alice's provider: 1.2 USDC
- Bob: 1996 USDC
- Bob's referrer: 2 USDC
- Bob's provider: 2 USDC
```
...
- ✅ You know which book the order is in (`bookId`)
...
Controller.claim(
    ClaimOrderParams({
        id: orderId,
        bookId: bookId,
        to: yourAddress  // Where to send proceeds
    }),
...
- ✅ Your provider and referrer received their fees
...
Controller.claim([
    ClaimOrderParams({ id: orderId1, bookId: bookId1, to: msg.sender }),
    ClaimOrderParams({ id: orderId2, bookId: bookId2, to: msg.sender }),
    ClaimOrderParams({ id: orderId3, bookId: bookId1, to: msg.sender })
], deadline);
```
...
- [Providers](/mms-api/concepts/providers) - understanding providers and referrers
- [Controller](/mms-api/concepts/controller) - the main trading interface
- [Place & cancel guide](/mms-api/guides/place-cancel) - how to place orders
