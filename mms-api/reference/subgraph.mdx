...
Represents an active limit order. Automatically deleted when fully claimed or canceled.

**Use this for querying active orders.**
...
<Note>
  Use `OpenOrder` for active orders. Use `Order` for complete history including
  fills, claims, and cancels.
</Note>
