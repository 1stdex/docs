...
Represents a taker trade (market order execution) on a book. Implements the `Trade` interface.

```graphql
type Take implements Trade {
  ...
  origin: Bytes! # Transaction origin (EOA)
  trader: Bytes! # Alias for origin (implements Trade interface)
...
type MakerFill implements Trade {
  ...
  order: OpenOrder # Reference to order (may be null if closed)
...
Represents a claim event when a maker withdraws filled orders from a book.
...
type Claim {
  ...
  # Remaining amounts after claim
  remainingClaimableUnitAmount: BigInt! # Remaining claimable units
  remainingCancelableUnitAmount: BigInt! # Remaining cancelable units
  logIndex: BigInt! # Log index in transaction
}
```
