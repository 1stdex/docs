...
<Note>
  **Important schema changes**: - `OpenOrder` entity renamed to `Order` -
  ...
  subgraph - Filter by `claimableUnitAmount_gt: "0"` or
  `cancelableUnitAmount_gt: "0"` to find active orders - New `Cancel` entity
  tracks all cancellation events - New `OrderMovement` interface allows querying
  takes, claims, and cancels together
</Note>
...
Query all orders for a user. Filter by `claimableUnitAmount` or `cancelableUnitAmount` to find active orders.
...
### Active orders with claimable amounts
...
### Active orders that can be canceled
