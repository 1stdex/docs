...
- Contract addresses (placeholders here; see [Addresses](/mms-api/reference/addresses))
...
const makeParams = encodeAbiParameters(makeTypes, [
  bookId,
  tick,
  makeQuote,
  provider,
...
```ts
// Cancel fully (leftQuoteAmount = 0 means cancel everything)
const cancelTypes = [
  { name: 'id', type: 'uint256' }, // OrderId
  { name: 'leftQuoteAmount', type: 'uint256' },
] as const;

const cancelParams = encodeAbiParameters(cancelTypes, [orderId, 0n]);
